class Bid < ApplicationRecord
  ...
  def validate_bidding_price
    lot.with_lock do
      ...
      if lot.bids.empty? && amount > lot.asking_price
        errors.add(:base, "Your bid must be equal to or lower than the asking price. The asking price is $#{lot.asking_price}")
      end

      if lot.bids.any?
        if amount >= lot.bids.last.amount
          errors.add(:base, "Your bid must be lower than the previous bids. The current bid is $#{lot.bids.last.amount}")
        end
      end
    end
  end
  ...
  def validate_company
    registration = AuctionRegistration.find_by(company_id: company_id, auction_id: lot.auction.id)
    unless registration
